...
var createFakeCanvas = require('./fake-canvas');
...
function retricon(str, opts) {
	...
	var c = createFakeCanvas(csize, csize)
	function drawOp(x, y, w, h) {
		c.fillRect(x, y, w, h)
	}
	...
	if (_.isString(opts.bgColor)) {
		...
		c.fillStyle = '#' + id.colors[opts.bgColor];
	}

	if (! _.isNull(opts.bgColor))
		drawOp(0, 0, csize, csize);

	if (_.isString(opts.pixelColor)) {
		c.fillStyle = opts.pixelColor;
	} else if (_.isNumber(opts.pixelColor)) {
		c.fillStyle = '#' + id.colors[opts.pixelColor];
	} else {
